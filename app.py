...
with st.sidebar:
    ...
    if not api_key:
        ...
        st.markdown(
            ...
            'background:#f0fdf4;padding:6px 10px;border-radius:6px;border:1px solid #bbf7d0;">'
    ...
    run_button = st.button(
        "▶  Run Pipeline",
        disabled=(not active_domain or st.session_state.running),
    )
...
# ─────────────────────────────────────────────────────────────────
    ...
    if st.session_state.logs:
        ...
        <div class="empty-state">
          ...
          <div class="empty-sub">Research → Patterns → Synthesis → Review → Report</div>
        </div>
        """, unsafe_allow_html=True)
