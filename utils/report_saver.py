...
def save_post(
    ...
    """
    Save the LinkedIn post candidates to posts_folder as a single .md file.

    All 3 candidates go into one file separated by clear dividers, so you
    can scroll through and pick the best one.

    Returns dict with post_path, posts_folder, base_name.
    """
    folder = Path(posts_folder).expanduser().resolve()
    folder.mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_domain = re.sub(r"[^\w\-]", "_", domain.lower())[:40]
    base_name = f"post_{safe_domain}_{ts}"

    # Build the file content — clean, no branding, easy to skim
    lines = [
        f"# LinkedIn Post Candidates — {domain}",
        "",
    ...
    post_path = folder / f"{base_name}.md"
    with open(post_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    return {
        "post_path":    str(post_path),
        "posts_folder": str(folder),
        "base_name":    base_name,
    }